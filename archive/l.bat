...
FOR %%A IN (-a --all) DO IF /I "%~1"=="%%A" SET "LIST_ALL=1" & GOTO next_arg
...
IF "%LIST_ALL%"=="1" GOTO list_all_action
...
:: 6. Helper Actions
:help_action
CALL "%SCRIPT_DIR%..\lib\help\%SCRIPT_FILE%" "%SCRIPT_NAME%" "%SCRIPT_FILE%" & ENDLOCAL & EXIT /B 0

:edit_action
CALL "%SCRIPT_DIR%..\lib\editor.bat" "%SCRIPT_PATH%" & ENDLOCAL & EXIT /B 0
